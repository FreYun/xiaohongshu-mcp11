...
// GetLoginQrcodeArgs 获取登录二维码的参数
type GetLoginQrcodeArgs struct {
	NotifySession string `json:"notify_session,omitempty" jsonschema:"可选。调用方的 session key（如 agent:bot1:main），扫码成功保存 cookie 后会通过该 session 回传通知"`
}

// PublishContentArgs 发布内容的参数（同时支持图片上传和文字配图两种模式）
type PublishContentArgs struct {
	...
	Products    []string `json:"products,omitempty" jsonschema:"商品关键词列表（可选）"`
	AccountID   string   `json:"account_id,omitempty" jsonschema:"bot 账号 ID（可选，每个服务实例已绑定端口）"`
}
...
// ManageNoteArgs 笔记管理参数
type ManageNoteArgs struct {
	FeedID string `json:"feed_id" jsonschema:"笔记ID"`
	Action string `json:"action" jsonschema:"操作类型: delete(删除)、pin(置顶)"`
}

// ReplyNotificationArgs 通知页回复参数
type ReplyNotificationArgs struct {
	CommentIndex int    `json:"comment_index" jsonschema:"要回复的评论序号（0-based），先用 get_notification_comments 获取列表"`
	Content      string `json:"content" jsonschema:"回复内容"`
}

// PublishLongformArgs 长文发布参数
type PublishLongformArgs struct {
	Title      string   `json:"title" jsonschema:"长文标题"`
	Content    string   `json:"content" jsonschema:"长文正文，支持多段落（段落间用换行分隔）"`
	Desc       string   `json:"desc,omitempty" jsonschema:"发布页描述（可选，不填则使用正文前800字）"`
